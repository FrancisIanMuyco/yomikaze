#!/usr/bin/env python3
"""fast_downloader — interactive Playwright manga downloader (CBZ/PDF/folder)."""
...
def main():
    ...
    fmt = input("Format (cbz/pdf/folder): ").strip().lower() or "cbz"
    workers = int(input("Workers (default 8): ").strip() or "8")
    ...
    with sync_playwright() as p:
        ...
        sel = input(f"\nEnter chapters to download (1-{len(chapters)}, e.g. 1-5 or 1,3,5 or ALL): ").strip().upper()
        if sel == "ALL":
            ...
            to_download = [chapters[i] for i in idxs if 0 <= i < len(chapters)]
        elif sel.isdigit():
            idx = int(sel) - 1
        ...
        print(f"\nDownloading {len(to_download)} chapters...")
        ...
        for ch in to_download:
            ...
            if fmt == "cbz":
                ...
                pdf = out_base / f"{slugify(ch['title'])}.pdf"
                pages = []
                for path in sorted(ch_dir.iterdir()):
                    try:
                        img = Image.open(path)
                ...
                if pages:
                    ...
                    print(f"  [✓] PDF ready ({len(pages)} pages)")
            else:

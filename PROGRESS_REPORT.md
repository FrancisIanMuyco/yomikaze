...
| 2026-08-26 | **Minimal update** — user requested no heavy scraping; library unchanged (131 titles / 7,075 chapters); proxies verified operational; laptop safe |
| 2026-08-29 | **Split rebuild fix** — `public/titles.json` naa 129 lang bisan 131 na ang `scraped.json` (2 ka titles sa Monday update wala pa live). Gi-rerun ang `scripts/split-scraped.mjs` → index 131 titles / 7,075 chapters, +2 title files. Commit + deploy. Library: mangafire 20 + asura 111 |
| 2026-08-30 | **Library EXPANSION — MangaFire REVIVED + Asura browse rail**: AsuraScans catalog na-import na ang tibuok nga appliances (browse sweep → 120 candidates, tanang dupes) → gidugang `browse` rail (paginated /browse/comics) sa `scraper/asurascans_scraper.py` (root copy). MangaFire.to Cloudflare gi-relax na → scraper gigamit ang bundled playwright chromium (kaniadto `channel="chrome"` kay absent sa Linux). **--fast mode** gidugang sa mangafire_catalog.py: API_RETRIES=1, walay backoff, walay slow response-capture fallback, walay context rotation — napaspas ~40× (kaniadto 91s/chapter, karon ~0.9s). Resulta: **155 → 177 titles** (+22 mangafire), chapters 7,551 → **8,839** (gi-strip ang 307 zero-page chapters per Rule 2), pages 294,984. Split rebuild + live deploy. |

| 2026-08-30 | **"Image unavailable" FIX** — MangaFire CDN (*.mfcdn*.xyz) hotlink-protected: gikinahanglan `Referer: mangafire.to` kung dili 403. Nahibal-an nga gitawag sa frontend ang `<proxy>/api/mfcdn?url=...` kay ang Cloudflare Worker (gikan sa UNA nga routing) lamang may `/mfcdn/<host>/<path>` → 404, UG ang CF worker IPs gi-block usab sa CDN (403). Fix: `api/mfcdn/index.js` → una-try ang Vercel's propio nga egress (referer-only ania usahay), fallback sa `MFCDN_PROXIES` (78 panigator nga proxies gi-refresh via `scripts/refresh-vercel-proxies.py`); `deploy.yml` → `VITE_IMAGE_PROXY_BASE = https://yomikaze.vercel.app` (dili na Cloudflare Worker). Na-verify: proxy 200 JPEG sa github.io + Vercel. |

| 2026-08-30 | **SPEED fix (parallel proxy)** — ang images kay naga-load na apan hinay (~24s): sequential proxy tries 25s-timeout ang hinungdan. `api/mfcdn/index.js` → direct + up to 8 proxies **in parallel**, first-win-wins, 8s cap per attempt, last-good proxy remembered. Measured: ~2.8s cold / ~1.8s warm. |
